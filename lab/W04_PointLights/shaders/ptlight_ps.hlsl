...
float4 main(InputType input) : SV_TARGET
{
  ...
  lightDir = normalize(input.position3D - lightPosition.xyz);
  //lightDir = normalize(input.position3D - lightPosition[0].xyz);
  ...
  if (lightIntensity > 0.0f)
  {
    ...
    //finalColour += saturate(diffuseColour[0] * lightIntensity);
