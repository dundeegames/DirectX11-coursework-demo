...
Lab8g::Lab8g() : m_Mesh(nullptr), m_SpecularShader(nullptr), m_TextureShader(nullptr),
                m_HorizontalBlurShader(nullptr), m_VerticalBlurShader(nullptr),
                m_Light(nullptr), m_RenderTexture1(nullptr), m_RenderTexture2(nullptr),
                m_OrthoMesh(nullptr), m_Cube(nullptr), m_AnotherCamera(nullptr)
{
...
Lab8g::~Lab8g()
{
  ...
  DELETE_OBJECT(m_HorizontalBlurShader);
  DELETE_OBJECT(m_VerticalBlurShader);
...
void Lab8g::init(HINSTANCE hinstance, HWND hwnd,
                  int screenWidth, int screenHeight, Input *in)
{
  ...
  m_HorizontalBlurShader->InitShader(L"shaders/horizontalBlur_vs.hlsl", L"shaders/horizontalBlur_ps.hlsl");

  m_VerticalBlurShader = new VerticalBlurShader(m_Direct3D->GetDevice(), hwnd);
  m_VerticalBlurShader->InitShader(L"shaders/verticalBlur_vs.hlsl", L"shaders/verticalBlur_ps.hlsl");
...
void Lab8g::ApplyHorBlur()
{
  ...
  m_HorizontalBlurShader->SetShaderParameters(m_Direct3D->GetDeviceContext(),
                                              worldMatrix, baseViewMatrix, orthoMatrix,
                                              m_RenderTexture1->GetShaderResourceView(),
                                              m_ScrWidth);
...
void Lab8g::DrawGeometry(Camera* camera)
{
  ...
  // Get Mesh position
  worldMatrix = XMMatrixTranslation(-1.5f, 0.0f, -2.0f);
  ...
  // Set shader parameters (matrices and texture)
  m_SpecularShader->SetShaderParameters(m_Direct3D->GetDeviceContext(), worldMatrix, viewMatrix, projectionMatrix,
    m_Cube->GetTexture(), m_Light, m_Camera);
  // Render object (combination of mesh geometry and shader process
