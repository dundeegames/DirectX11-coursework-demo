...
BaseShader::BaseShader(ID3D11Device* device, HWND hwnd) : m_device(device), m_hwnd(hwnd),
                                                          m_vertexShader(nullptr), m_pixelShader(nullptr),
                                                          m_hullShader(nullptr), m_domainShader(nullptr),
...
BaseShader::~BaseShader()
{
  ...
  RELEASE_OBJECT(m_pixelShader);
  RELEASE_OBJECT(m_vertexShader);
  RELEASE_OBJECT(m_hullShader);
  RELEASE_OBJECT(m_domainShader);
  RELEASE_OBJECT(m_geometryShader);
